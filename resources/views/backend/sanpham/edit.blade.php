...
<form method="post" action="{{ route('backend.sanpham.update', ['id' => $sp->sp_ma]) }}" enctype="multipart/form-data">
    ...
    <div class="form-group">
            <label for="sp_maybo">Loại máy tính</label>
            <select name="sp_maybo" class="form-control">
                <option value="0" {{ old('sp_maybo', $sp->sp_maybo) == 0 ? "selected" : "" }}>Laptop</option>
                <option value="1" {{ old('sp_maybo', $sp->sp_maybo) == 1 ? "selected" : "" }}>Máy bộ</option>
            </select>
    </div>
